...
zip_file_Location <- 'C:/MyApps/dapTbElectricDublinBus/_pipeline/gtfs.zip'
download.file(url = file, destfile = zip_file_Location)

# Unzip the data to a raw folder
# This will create a series of txt files for each GTFS 'table'
unzip(
  zipfile = zip_file_Location,
  exdir = 'C:/MyApps/dapTbElectricDublinBus/_pipeline/raw'
  )
...
depot_mappings <- read.csv('C:/MyApps/dapTbElectricDublinBus/_pipeline/depot_mapping.csv', stringsAsFactors = F)
